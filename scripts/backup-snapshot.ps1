...
function Assert-SafeTrackedSource {
    ...
    if ($TrackedPaths -contains '.npmrc') {
        $NpmrcLines = @(& git -C $RepoRoot show HEAD:.npmrc)
        if ($LASTEXITCODE -ne 0) {
            throw "Unable to inspect the tracked .npmrc."
        }
        $UnsupportedNpmrc = @($NpmrcLines | Where-Object {
            $_ -notmatch '^\s*$' -and
            $_ -notmatch '^#' -and
            $_ -notmatch '^node-linker=(hoisted|isolated|pnp)$' -and
            $_ -notmatch '^strict-peer-dependencies=(true|false)$'
        })
        if ($UnsupportedNpmrc.Count -gt 0) {
            throw "Refusing to archive .npmrc with non-allowlisted settings."
        }
    }
}
