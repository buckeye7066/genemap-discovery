...
describe('PhenotypeSearchService.applyAuthoritativeData', () => {
  it('overlays a verified gene record and validated HPO id, tagging provenance', () => {
    ...
    const authGenes = {
      RUNX1: {
        ...
        genomeBuild: 'GRCh38', species: 'Homo sapiens', taxId: 9606,
        mapLocation: '21q22.12', source: 'MyGene.info',
      },
    };
    ...
    expect(g.species).toBe('Homo sapiens');
    expect(g.taxId).toBe(9606);
  ...
  it('keeps AI data clearly labeled when no authoritative record exists', () => {
    ...
    expect(g.species).toBeNull();
    expect(g.taxId).toBeNull();
