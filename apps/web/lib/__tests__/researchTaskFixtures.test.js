...
describe('legacy research example parser', () => {
  ...
  it('clears stale focus identities when an example has no focus', () => {
    expect(researchFocusControls()).toEqual({
      focusKind: 'none',
      focusConceptId: '',
      focusHpoId: '',
    });
    expect(researchFocusControls({ kind: 'hpo', identifier: 'HP:0001250' })).toEqual({
      focusKind: 'hpo',
      focusConceptId: '',
      focusHpoId: 'HP:0001250',
    });
  });
});
