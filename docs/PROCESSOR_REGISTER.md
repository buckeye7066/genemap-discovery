...
Last code-flow review: **2026-08-25**, including the source-grounded network
adapter added on pull request #190.
...
| STRING public API | `string-db.org` | **Yes** — authenticated `POST /genomics/gene-network` from gene comparison | `geneNetwork.js` (`STRING_NETWORK_ENDPOINT` and `getGeneNetwork`); route `genomics.js` (`/gene-network`); client `geneNetworkClient.js` | Two to ten policy-filtered public gene symbols; fixed human taxon `9606`, functional network type, bounded source threshold, zero to five added neighbors, and a caller identifier. No phenotype text, variants, patient records, or model prompt is sent. | The UI links the [official API documentation](https://string-db.org/help/api/), labels query versus added nodes, exposes channel scores and retrieval time, and states that associations are neither causal nor clinical. |
| Open Targets Platform GraphQL API v4 | `api.platform.opentargets.org` | **Yes** — `/genomics/association-evidence` | `associationEvidence.js:12`, POST `:509-517`, query `:493-508`, variable `:467-469` | GraphQL document with variable `diseaseId` = the MONDO id with `:`→`_`. Candidate gene symbols are filtered **locally** (`:544-548`) and are not sent. Provider scores are exposed only as labeled components inside the computed source claim; they are not calibrated probabilities and do not change the claim class. | The UI and exports retain source, release/retrieval metadata, direct records, and score-component identity. |
