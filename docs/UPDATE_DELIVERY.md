...
Known desktop application limitation: the existing file-based renderer sends Origin:null, which the live API currently rejects (health probe500, versus200 for https://localhost). The installer updater runs in the main process and is independent of that API path. Use the web app for authenticated work pending a separate supported desktop-origin fix; no CORS widening or storage migration is included here. The new public bootstrap release description carries this limitation.
