...
GeneMap native prerequisite: the installed Capacitor CLI must run in your workspace. Existing CI uses an isolated pinned CLI because some hoisted installations have a CJS/ESM incompatibility. This dispatcher never installs that workaround; a broken installed CLI stops the build and must be repaired separately.
